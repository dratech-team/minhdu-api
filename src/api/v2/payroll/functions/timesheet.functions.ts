...
import {SalaryUtils} from "./salary.functions";

export namespace TimeSheetUtils {
  export const timesheet = (payroll: PayrollEntity | any, isExport?: boolean) => {
    const diff = rageDateTime(firstDatetime(payroll.createdAt), lastDatetime(payroll.createdAt));
    const range = [];
    let total = 0;
    const dayoffs = SalaryUtils.dayoffUniq(payroll.dayoffs)?.map(e => ({
      unit: DatetimeUnit.DAY,
      partial: e.partial,
      datetime: e.datetime
    }));
    const absents = SalaryUtils.absentUniq(payroll.absents)?.map(e => ({
      unit: e.setting.unit,
      partial: e.partial,
      datetime: e.datetime
    })).concat(dayoffs);
...
function rageDateTime(startedAt: Date, endedAt: Date, type: "days" | "months" | "years" = "days"): moment.Moment[] {
  const range = [];
